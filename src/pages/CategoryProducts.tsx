...
function ProductRow({ product, index, isLast }: { product: ProductListItem; index: number; isLast: boolean }) {
  ...
  return (
    <Link
      ...
      ref={ref as any}
      className={[
        'group relative flex flex-col md:grid md:grid-cols-12 items-center gap-10 md:gap-14 py-14 md:py-20',
        reverse ? 'md:[&>*:first-child]:col-start-8' : '',
        'after:absolute after:left-1/2 after:-translate-x-1/2 after:bottom-0 after:h-px after:w-[90%] after:bg-gradient-to-r after:from-transparent after:via-slate-200 dark:after:via-slate-800 after:to-transparent',
        isLast ? 'after:opacity-0' : 'after:opacity-100',
      ...
      <section className="py-20 bg-gradient-to-b from-white via-slate-50 to-white dark:from-slate-900 dark:via-slate-950 dark:to-slate-900 relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          ...
          <div className="flex flex-col gap-16 md:gap-24">
            {products.map((p,i) => (
              <ProductRow key={p.id} product={p} index={i} isLast={i===products.length-1} />
            ))}
          </div>
